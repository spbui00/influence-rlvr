...
from influence_rlvr.prompts import extract_gsm8k_target
...
from influence_rlvr.training_data import (
    NUMINA_DATASET,
    build_training_script_math_prompt,
...
def run_gsm8k_eval(
    ...
    *,
    ...
    for i, ex in enumerate(eval_ds):
        messages = build_training_script_math_prompt(ex["question"])
    ...
    *,
    ...
    for i, ex in enumerate(eval_ds):
        ...
        if not str(gold).strip():
            continue
        messages = build_training_script_math_prompt(ex["problem"])
        completion_texts = _generate_eval_completions(
            model,
            tokenizer,
            messages,
...
Mixed Numina + TACO (same Phase-1 objective as main_pipeline MIXED_GRPO):
  python training_script.py --mixed --skip-eval \\
    --n-numina 300 --n-taco 300 --mixed-shuffle-seed 42 \\
    --output-dir ./outputs/mixed_run/rlvr-output --seed 42 \\
    --g-train 16 --per-device-batch 8 --grad-accum 2
"""
...
def parse_args():
    ...
    p.add_argument(
        "--numina-dataset",
        default=NUMINA_DATASET,
        help=f"HF dataset id for math (default: {NUMINA_DATASET}).",
    )
    p.add_argument(
        "--n-numina",
        type=int,
        default=300,
        help="Numina train slice size (mixed mode only).",
    )
    p.add_argument(
        "--n-taco",
        type=int,
    ...
    p.add_argument(
        ...
        type=int,
        default=42,
        help="Seed for shuffling concatenated Numina+TACO (mixed mode only).",
    )
    p.add_argument(
        "--n-math",
        type=int,
        default=0,
        help="GSM8K train examples: first N rows, or 0 (default) for the full train split (~7.5k). Ignored with --mixed.",
    )
...
def main():
    ...
    training_data = load_training_data_bundle(args)
    ...
    reward_funcs = training_data.reward_funcs
